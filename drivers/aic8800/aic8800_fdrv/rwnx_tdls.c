                      ...
                      u16 status_code, struct sk_buff *skb)
{
    ...
    case WLAN_PUB_ACTION_TDLS_DISCOVER_RES:
        skb_put(skb, RWNX_TDLS_DISCOVER_RESP_LEN(mgmt));
        mgmt->u.action.category = WLAN_CATEGORY_PUBLIC;
        RWNX_TDLS_ACTION_CODE(mgmt) = WLAN_PUB_ACTION_TDLS_DISCOVER_RES;
        RWNX_TDLS_DISCOVER_RESP(mgmt).dialog_token = dialog_token;
        RWNX_TDLS_DISCOVER_RESP(mgmt).capability =
            cpu_to_le16(rwnx_get_tdls_sta_capab(rwnx_vif, status_code));
        break;
    default:
        return -EINVAL;
